...
PXE-Hauptkonfigurationsdatei default anpassen. Wir fügen folgende CentOS Stanza hinzu:
```plaintext
[2.7.2-RELEASE][admin@pfSense.home.arpa]/tftpboot: cat pxelinux.cfg/default
DEFAULT menu.c32
PROMPT 0
TIMEOUT 300
...
LABEL centos
    ...
    APPEND ip=dhcp ks=http://192.168.100.2/tftpboot/centos/ks-centos.cfg repo=http://192.168.100.2/iso/CentOS-Stream-9-latest-x86_64-dvd1.iso
```

In diesem Beispiel haben wir nur CentOS aufgestellt deshalb reicht die CentOS Stanza aus. Später, wenn alle anderen Betriebssysteme vorbereitet sind, wird die vollständige `pxelinux.cfg/default` Datei so aussehen:
...
LABEL fedora
    ...
    APPEND ip=dhcp ks=http://192.168.100.2/tftpboot/fedora/ks-fedora.cfg repo=http://192.168.100.2/iso/Fedora-Everything-netinst-x86_64-40-1.14.iso

LABEL ubuntu
    MENU LABEL Ubuntu
    KERNEL ubuntu/vmlinuz
    INITRD ubuntu/initrd.img
    APPEND ip=dhcp ks=http://192.168.100.2/tftpboot/ubuntu/ks-ubuntu.cfg repo=http://192.168.100.2/iso/ubuntu-24.04.1-live-server-amd64.iso

LABEL debian
    MENU LABEL Debian
    KERNEL debian/vmlinuz
    INITRD debian/initrd.img
    APPEND ip=dhcp ks=http://192.168.100.2/tftpboot/debian/ks-debian.cfg repo=http://192.168.100.2/iso/debian-12.7.0-amd64-netinst.iso

LABEL kali
    MENU LABEL Kali
    KERNEL kali/vmlinuz
    INITRD kali/initrd.img
    APPEND ip=dhcp ks=http://192.168.100.2/tftpboot/kali/ks-kali.cfg repo=http://192.168.100.2/iso/kali-linux-2024.3-installer-netinst-amd64.iso
```
---
6. **ISO Images auf pfSense hochladen** (brauchen wir):

Von Windows aus den Ordner `/usr/local/www/iso` erstellen:
```powershell
...
---
6. **PXE-Boot testen** (brauchen wir):
