...
# Tresore (Vaults) und Passwortarten
...
Da `yum` und `apt` Root-Rechte benötigen, muss das Become-Passwort in der `vault.yml`-Datei gespeichert werden.
...
Damit wird das Become-Passwort sicher in einer verschlüsselten Datei gespeichert und kann für die Privilegienerhöhung in Ansible verwendet werden.
