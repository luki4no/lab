...
```plaintext
# PXE-Boot Reihenfolge mit Dateien
...
3. NFS-, HTTP- oder FTP-Server
   ...
   |   - Unverpackte ISO-Inhalte.                  |
   | - Reihenfolge der genutzten Dateien:          |
   |   - Kickstart- oder Preseed-Datei (z. B.      |
   |     ks.cfg, preseed.cfg).                     |
   |   - Pakete und OS-Installationsdateien        |
   |     (z. B. für CentOS: /BaseOS/*, /AppStream/*|
   |     für Debian: /pool/*).                     |
   | - Wichtig: ISO-Inhalt muss entpackt werden,   |
   |   außer bei NFS, das direkte Verlinkung       |
   |   unterstützt.                                |
   +-----------------------------------------------+
