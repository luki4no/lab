...
2. TFTP-Server
   +-----------------------------------------------+
   | - Überträgt die Boot-Dateien wie:             |
   |   - pxelinux.0 (BIOS) oder grubx64.efi (EFI). |
   |   - Kernel (vmlinuz) und Initrd (initrd.img). |
   | - Wird nur für den initialen Bootprozess      |
   |   genutzt.                                    |
   +-----------------------------------------------+
                      │
                      ▼
3. NFS-, HTTP- oder FTP-Server
   ...
   |   - Kickstart-, Preseed-Dateien.              |
   |   - ISO-Inhalt der Distribution.              |
