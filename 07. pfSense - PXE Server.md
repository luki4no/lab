...
1. **DHCP-Server**: Stellt dem Client die Netzwerkkonfiguration bereit, wie z. B. die IP-Adresse, das Gateway und den Speicherort der Boot-Datei. Zusätzlich können folgende **PXE-spezifische DHCP-Optionen** verwendet werden:  
   ...
   **pfSense-Konfiguration**:  
   ...
   - Zusätzlich gibt es die Option **Custom DHCP Options**, in der die Optionen 66 (TFTP-Server) und 67 (Boot-Dateiname) manuell konfiguriert werden können. Diese manuelle Konfiguration ist jedoch nicht zwingend erforderlich, da pfSense anhand der oben genannten Einstellungen automatisch erkennt, ob es sich um einen BIOS- oder EFI-Client handelt.
