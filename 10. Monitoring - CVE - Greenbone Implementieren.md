...
```
```bash
sed -i '/openvasd:/,/ports:/s/^\( *\)ports:/# \1ports:/' docker-compose.yml
```
```bash
sed -i '/openvasd:/,/environment:/!b;/LISTENING:/c\      LISTENING: 0.0.0.0:80' docker-compose.yml
```

Falls die `docker-compose.yml` Datei schon mit `127.0.0.1:9392:80` gestartet wurde, einfach wie oben beschrieben anpassen und neustarten:
