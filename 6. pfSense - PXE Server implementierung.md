...
- [Installationsdateien auf pfSense transferieren](#installationsdateien-auf-pfsense-transferieren)
  ...
  - [OS-Ordner](#os-ordner)
  - [Ordner für Automatisierungsdateien](#ordner-für-automatisierungsdateien)
  - [ISO-Images transferieren](#iso-images-transferieren)
  - [ISO-Image in pfSense entpacken](#iso-image-in-pfsense-entpacken)
  - [Berechtigungen anpassen](#berechtigungen-anpassen)
- [PXE Dienste](#pxe-dienste)
...
- [Kernel (vmlinuz) und Initial RAM Disk Image (initrd.img) importieren](#kernel-vmlinuz-und-initial-ram-disk-image-initrdimg-importieren)
  ...
  - [OS-Ordner erstellen und kopieren](#os-ordner-erstellen-und-kopieren)
...
- [Abschließen](#abschließen)
  - [Überprüfen und Bestätigen der DHCP-Einstellungen auf pfSense](#überprüfen-und-bestätigen-der-dhcp-einstellungen-auf-pfsense)
  - [Firewall-Regeln konfigurieren](#firewall-regeln-konfigurieren)
  - [TFTP-Konfiguration überprüfen](#tftp-konfiguration-überprüfen)
  - [HTTP/FTP/NFS-Dienste einrichten und testen](#httpftp-nfs-dienste-einrichten-und-testen)
  - [PXE-Boot-Menü-Konfigurationen aktualisieren](#pxe-boot-menü-konfigurationen-aktualisieren)
  - [PXE-Boot testen](#pxe-boot-testen)
  - [Fehlerbehebung](#fehlerbehebung)
  - [Dokumentation und Backups](#dokumentation-und-backups)
- [Backup](#backup)
  - [Backup /tftpboot Struktur](#backup-tftpboot-struktur)
  - [Restore /tftpboot Struktur](#restore-tftpboot-struktur)
...
Order für Automatisierungsdateien
```powershell
ssh admin@192.168.100.2 mkdir -p /usr/local/www/pxe/automation
```

ISO-Images transferieren:
```powershell
cd C:\lab\vm\iso-images
```
```powershell
scp "C:\lab\vm\iso-images\CentOS-Stream-9-latest-x86_64-dvd1.iso" admin@192.168.100.2:/usr/local/www/pxe/centos
```
```powershell
scp "C:\lab\vm\iso-images\Fedora-Server-dvd-x86_64-41-1.4.iso" admin@192.168.100.2:/usr/local/www/pxe/fedora
```
```powershell
scp "C:\lab\vm\iso-images\ubuntu-24.04.1-live-server-amd64.iso" admin@192.168.100.2:/usr/local/www/pxe/ubuntu
```
```powershell
scp "C:\lab\vm\iso-images\debian-12.7.0-amd64-netinst.iso" admin@192.168.100.2:/usr/local/www/pxe/debian
```
```powershell
scp "C:\lab\vm\iso-images\kali-linux-2024.3-installer-netinst-amd64.iso" admin@192.168.100.2:/usr/local/www/pxe/kali
```
```powershell
...
```bash
bsdtar -C /usr/local/www/pxe/fedora -xf /usr/local/www/pxe/fedora/Fedora-Server-dvd-x86_64-41-1.4.iso
```
...
* Other DHCP Options
  ...
  * Network Booting > Display Advanced
    ...
    * UEFI 64 bit File Name: `http://192.168.100.10/pxe`
