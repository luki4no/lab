...
- [Abschließen](#abschließen)
  ...
  - [4. HTTP/FTP/NFS-Dienste einrichten und testen](#4-httpftpnfs-dienste-einrichten-und-testen)
...
```
```powershell
scp "C:\lab\vm\iso-images\*" admin@192.168.100.2:/usr/local/www/pxe
```
```powershell
...
```plaintext
http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso
```
```plaintext
http://192.168.100.2/pxe/Fedora-Server-dvd-x86_64-41-1.4.iso
```
...
LABEL Install CentOS
  ...
  APPEND initrd=/centos/initrd.img inst.repo=http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso inst.ks=http://192.168.100.2/pxe/automation/kickstart-centos.cfg

LABEL Install Fedora
  MENU LABEL Install Fedora
  KERNEL /fedora/vmlinuz
  APPEND initrd=/fedora/initrd.img inst.repo=http://192.168.100.2/pxe/Fedora-Server-dvd-x86_64-41-1.4.iso inst.ks=http://192.168.100.2/pxe/automation/kickstart-fedora-server.cfg
```
...
menuentry 'Install CentOS Stream 9' {
  linuxefi /centos/vmlinuz inst.repo=http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso inst.ks=http://192.168.100.2/pxe/automation/kickstart-centos.cfg quiet
  initrdefi /centos/initrd.img
}

menuentry 'Test this media & install CentOS Stream 9' {
  linuxefi /centos/vmlinuz inst.repo=http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso rd.live.check quiet
  initrdefi /centos/initrd.img
}

submenu 'Troubleshooting -->' {
  menuentry 'Install CentOS Stream 9 in text mode' {
    linuxefi /centos/vmlinuz inst.repo=http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso inst.text quiet
    initrdefi /centos/initrd.img
  }

  menuentry 'Rescue a CentOS Stream system' {
    linuxefi /centos/vmlinuz inst.repo=http://192.168.100.2/pxe/CentOS-Stream-9-latest-x86_64-dvd1.iso inst.rescue quiet
    initrdefi /centos/initrd.img
  }
}
```
