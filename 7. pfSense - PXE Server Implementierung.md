...
```
```powershell
scp "C:\lab\vm\iso-images\debian-12.7.0-amd64-netinst.iso" admin@192.168.100.2:/usr/local/www/pxe/iso
```
```powershell
...
* Other DHCP Options
  ...
  * Network Booting > Display Advanced
    ...
    * UEFI HTTPBoot URL: `http://192.168.100.2/pxe/iso/` <---- Optional: hier kann ein Pfad zu einer LIVE ISO angeben werden. Nicht nötig.
