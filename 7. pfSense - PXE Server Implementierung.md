...
Skript auf pfsense transferieren und die ISO-Images direkt auf pfsense in `/usr/local/www/pxe/iso` herunterladen:
```bash
...
```
Heruntergeladene ISO-Images vom lokalen Rechner transferieren:
```powershell
